...
/*   Updated: 2022/10/24 15:07:04 by hseven           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	skip_quotes(t_input *input, int *i, char quote)
{
	...
	if (input->line[*i] == '\0')
		flag = 0;
	while ((is_space_or_null(input->line[*i]) == 0 || flag)
		&& is_special_char(input->line[*i]) == 0)
	{
		if (input->line[*i] == quote)
			flag = !flag;
		else if (!flag && is_special_char(input->line[*i]))
			input->args_size++;
		(*i)++;
	}
